# Distributed ML Training — Design Document (Part 4–5: Compute)
...
3. When every shard-owning worker has reported a gradient for the current epoch, I
   aggregate: `avg_grad = Σ gradients / Σ sample_counts`, then step
...
`ml/model` — one atomic `Put`, replicated to a majority. If the coordinator crashes, I
...
than on one local disk, it survives the loss of any single node — the etcd-style
...
its accuracy on the training data clears a threshold — exercising the whole data-parallel
